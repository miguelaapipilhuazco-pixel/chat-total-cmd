...
export function obtenerInterfaz(userAgent, config, colorFondo, efectoBlur, estiloBorder, radioBordes) {
  ...
  if (userAgent.includes('android') || userAgent.includes('watch')) {
    radioBordes = '50px'; colorFondo = 'rgba(10, 10, 10, 0.95)'; efectoBlur = ''; estiloBorder = 'none';
  ...
  /* RENDERS DE ALTA CALIDAD UNIFICADOS (Volumen, Relieve y Sombreado 3D Fotorrealista) */
  ...
  /* RESALTE DE VOLUMEN MULTIMEDIA EN CADA SÍMBOLO INDEPENDIENTE */
  .braille-ico { color: rgba(255, 255, 255, 0.75) !important; font-size: 34px; font-weight: bold; line-height: 1; height: 34px; filter: drop-shadow(0 3px 5px rgba(0,0,0,0.6)); /* Sombra 3D densa */ }
  .braille-ico:hover { color: #fbbf24 !important; filter: drop-shadow(0 0 12px rgba(251,191,36,0.85)) drop-shadow(0 3px 5px rgba(0,0,0,0.6)); }

  .senas-ico { font-size: 28px; width: 36px; height: 36px; filter: drop-shadow(0 3px 5px rgba(0,0,0,0.6)); /* Calidad de relieve integrada */ }
  .senas-ico:hover { transform: scale(1.18); filter: drop-shadow(0 0 12px rgba(245,158,11,0.75)) drop-shadow(0 3px 5px rgba(0,0,0,0.6)) !important; }

  .texto-ico { font-size: 28px; font-weight: 900; font-family: 'Times New Roman', serif; color: rgba(255, 255, 255, 0.75) !important; letter-spacing: -2px; width: 36px; height: 36px; filter: drop-shadow(0 3px 5px rgba(0,0,0,0.6)); /* Relieve de tipografia */ }
  .texto-ico:hover { color: #f59e0b !important; filter: drop-shadow(0 0 12px rgba(245,158,11,0.85)) drop-shadow(0 3px 5px rgba(0,0,0,0.6)); }

  .flecha-ico { font-size: 15px; font-weight: 900; color: rgba(255, 255, 255, 0.4); width: 20px; height: 20px; text-align: center; filter: drop-shadow(0 2px 3px rgba(0,0,0,0.5)); }
  .flecha-ico:hover { color: #fbbf24; filter: drop-shadow(0 0 5px rgba(251,191,36,0.8)); }
  
  /* MICROFONO FOTORREALISTA CORREGIDO SIN SUB-INVERSIONES DE CONTENEDOR */
  .voz-ico { font-size: 30px; filter: drop-shadow(0 3px 5px rgba(0,0,0,0.6)); }
  .voz-ico:hover { filter: drop-shadow(0 0 12px rgba(251,191,36,0.75)) drop-shadow(0 3px 5px rgba(0,0,0,0.6)) !important; }
