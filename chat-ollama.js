...
export async function consultarOllama(message, prompt, config) {
  try {
    ...
    // IP CORREGIDA: Sincronización exacta con el puerto nativo de Ollama en Windows
    const res = await fetch('http://127.0.0', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: 'llama3', // Reemplaza por 'phi3' o 'gemma' si usas otro modelo local
        messages: [
    ...
    if (data && data.message && data.message.content) {
      ...
      // CAPA DE AUTO-MUTACIÓN DE NIVEL 4: Si Ollama genera código, reescribe el disco duro solo
      if (prompt.includes('modif') || prompt.includes('cambia') || prompt.includes('actualiz') || prompt.includes('interfaz') || prompt.includes('diseño') || prompt.includes('codigo')) {
